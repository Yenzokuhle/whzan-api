...
namespace Whzan_API.Services.Concrete
{
    public class FavouriteRepository : IFavouriteRepository
    {
        ...
        public FavouriteRepository(WhzanCatalogDBContext whzanCatalogDBContext) 
        { 
        ...
        {
            var favourits = await _whzanCatalogDBContext.Product
                    .Include(p => p.ProductImages)
                        .ThenInclude(pi => pi.Image)
                    .Where(p => p.IsFavourite == true)
                    .ToListAsync();

            var favtDTOs = favourits.Select(product => new ProductResponse()
            {
                Id = product.Id,
                Name = product.Name,
